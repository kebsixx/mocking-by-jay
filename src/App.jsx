  ...
  return (
    <nav className="fixed top-0 left-1/2 -translate-x-1/2 z-50 mt-4 w-[calc(100%-32px)] max-w-3xl">
      <div className="flex items-center justify-between px-5 py-3 rounded-full bg-white/5 backdrop-blur-2xl border border-white/10 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
        <a href="#" className="text-sm font-semibold tracking-tight text-white/90 select-none">
        ...
        >
          <span className={`block w-5 h-[1.5px] bg-muted transition-all duration-300 ${menuOpen ? "rotate-45 translate-y-[5.5px]" : ""}`} />
          <span className={`block w-5 h-[1.5px] bg-muted transition-all duration-300 ${menuOpen ? "opacity-0" : ""}`} />
          <span className={`block w-5 h-[1.5px] bg-muted transition-all duration-300 ${menuOpen ? "-rotate-45 -translate-y-[5.5px]" : ""}`} />
        </button>
      </div>
      ...
      >
        {[...links, { label: "Book", href: "#book", highlight: true }].map((link, i) =>
          link.highlight ? (
            <button
              key={link.label}
...
function Hero({ onOpenBooking }) {
  ...
  return (
    <section id="hero" ref={ref} className="relative min-h-[100dvh] flex flex-col items-center justify-center px-6 pt-24 pb-16 overflow-hidden">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full bg-red-accent/5 blur-[120px] pointer-events-none" />
      <div className={`relative z-10 flex flex-col items-center text-center max-w-4xl mx-auto transition-all duration-[900ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
        visible ? "translate-y-0 opacity-100 blur-0" : "translate-y-12 opacity-0 blur-[2px]"
      }`}>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          <a href="#artists" className="px-8 py-3.5 rounded-full text-sm font-medium text-white/70 border border-white/10 transition-all duration-300 hover:text-white hover:border-white/20 active:scale-[0.97]">
            Our Artists
          </a>
        </div>
      </div>
      <div aria-hidden="true" className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-muted-2">
        <span className="text-[10px] font-medium tracking-[0.15em] uppercase">Scroll</span>
        <div className="w-[1px] h-8 bg-gradient-to-b from-muted-2 to-transparent animate-pulse" />
      </div>
    </section>
  );
}
...
/* ───────────────────────────────────────────
   ...
   ─────────────────────────────────────────── */
const values = [
  { label: "Artists First", desc: "Every decision starts with the creator. Their vision leads, we follow." },
  { label: "Real Sound", desc: "No formulas. No trends. Just honest music that moves people." },
  { label: "Built Different", desc: "Independent roots, global reach. We move at our own pace." },
];
      ...
      <Reveal>
        <div className="max-w-3xl mx-auto text-center mb-16">
          ...
          <p className="text-muted leading-relaxed max-w-2xl mx-auto">
            Mocking by Jay was built on the belief that real music comes from real freedom.
            We don't box our artists in. We give them the space, the tools, and the trust
            to create work that matters. From underground studios to sold-out stages,
            we're here to push culture forward.
          </p>
        </div>
      </Reveal>
        ...
        {values.map((v, i) => (
          <Reveal key={v.label} delay={i * 100}>
            <div className="p-8 rounded-2xl bg-surface border border-white/5 hover:border-red-accent/20 transition-all duration-500 group">
              ...
              <h3 className="text-lg font-semibold text-white mb-2 group-hover:text-red-accent transition-colors duration-300">{v.label}</h3>
...
/* ───────────────────────────────────────────
   ...
   ─────────────────────────────────────────── */
const artists = [
  { name: "Kai Lennox", genre: "Hip-Hop / Trap", tag: "Rising", img: "https://picsum.photos/seed/kai-lennox/600/700" },
  { name: "Mira Valent", genre: "R&B / Soul", tag: "Featured", img: "https://picsum.photos/seed/mira-valent/600/700" },
  { name: "Lex Vaughn", genre: "Drill / UK Rap", tag: "New Signing", img: "https://picsum.photos/seed/lex-vaughn/600/700" },
  { name: "Noor Ashraf", genre: "Alternative Hip-Hop", tag: "Latest Album", img: "https://picsum.photos/seed/noor-ashraf/600/700" },
  { name: "Cruz Mendez", genre: "Latin Trap", tag: "On Tour", img: "https://picsum.photos/seed/cruz-mendez/600/700" },
];
      ...
      <Reveal>
        <div className="max-w-3xl mx-auto text-center mb-16">
          ...
          <p className="text-muted leading-relaxed max-w-xl mx-auto">
            From rising talent to established voices. Each artist brings something
            raw and real to the table.
          </p>
        </div>
      </Reveal>
            ...
            <div className="group relative rounded-2xl bg-surface border border-white/5 overflow-hidden hover:border-red-accent/20 transition-all duration-500">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[10px] font-medium tracking-[0.15em] uppercase text-red-accent/80">{artist.genre}</span>
                  <span className="w-1 h-1 rounded-full bg-red-accent/40" />
                  <span className="text-[10px] font-medium text-muted-2">{artist.tag}</span>
                </div>
        ...
        <Reveal>
          <div>
            ...
            <p className="text-muted leading-relaxed mb-8">
              Our studio is equipped with top-tier analog and digital gear. Whether
              you are tracking, mixing, or producing, you get the space and tools
              to bring your vision to life.
            </p>
            <ul className="space-y-3 mb-10">
              {["SSL Console & outboard gear", "Isolated live room & vocal booth", "Pro Tools HDX + Ableton Live", "On-site engineer support"].map((item) => (
                <li key={item} className="flex items-center gap-3 text-sm text-muted">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-red-accent shrink-0">
            ...
            >
              Book Studio Time
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="ml-2">
                <line x1="5" y1="12" x2="19" y2="12" />
                <polyline points="12 5 19 12 12 19" />
              </svg>
            </button>
...
/* ───────────────────────────────────────────
   ...
   ─────────────────────────────────────────── */
const tracks = [
  { title: "Midnight Run", artist: "Kai Lennox", plays: "1.2M", color: "from-red-accent/20" },
  { title: "Velvet", artist: "Mira Valent", plays: "892K", color: "from-white/10" },
  { title: "Block Star", artist: "Lex Vaughn", plays: "2.4M", color: "from-red-accent/20" },
  { title: "Waves", artist: "Noor Ashraf", plays: "654K", color: "from-white/10" },
];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        {tracks.map((track, i) => (
          <Reveal key={track.title} delay={i * 80}>
            <div className={`group flex items-center gap-4 p-5 rounded-2xl bg-surface border border-white/5 hover:border-red-accent/20 transition-all duration-500`}>
              {/* Album art placeholder */}
              <div className={`w-14 h-14 rounded-lg bg-gradient-to-br ${track.color} to-surface-2 shrink-0 flex items-center justify-center border border-white/5`}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-muted-2 group-hover:text-red-accent transition-colors duration-300">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="text-sm font-semibold text-white truncate group-hover:text-red-accent transition-colors duration-300">{track.title}</h4>
                <p className="text-xs text-muted-2">{track.artist}</p>
              </div>
              {/* Plays + play button */}
              <div className="flex items-center gap-3">
                <span className="text-[11px] text-muted-2 font-mono">{track.plays}</span>
                <button className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center hover:bg-red-accent transition-all duration-300 group/btn active:scale-[0.92]">
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor" className="text-muted-2 group-hover/btn:text-white transition-colors duration-300 ml-[1px]">
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-surface border border-white/10 rounded-3xl p-8 md:p-10 shadow-2xl animate-in fade-in zoom-in-95 duration-300">
        ...
        {step === "form" ? (
          <>
            <div className="mb-8">
              <h2 className="text-2xl font-bold text-white mb-2">Book an Artist</h2>
              <p className="text-sm text-muted">Tell us about your project. We will get back to you within 48 hours.</p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-muted-2 mb-1.5">Name *</label>
                  <input
                    type="text"
                    name="name"
                    value={form.name}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-muted-2 mb-1.5">Phone</label>
                  <input
                    type="tel"
                    name="phone"
                    value={form.phone}
                ...
                <div>
                  <label className="block text-xs font-medium text-muted-2 mb-1.5">Event Type *</label>
                  <select
                    name="eventType"
                  ...
                  >
                    ...
                    {eventTypes.map((t) => (
                      <option key={t} value={t} className="bg-ink-3">{t}</option>
                    ))}
                  </select>
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-medium text-muted-2 mb-1.5">Preferred Date</label>
                <input
                  type="date"
                  name="date"
                  value={form.date}
              ...
              <div>
                <label className="block text-xs font-medium text-muted-2 mb-1.5">Message</label>
                <textarea
                  name="message"
                  value={form.message}
                  onChange={handleChange}
          ...
          <div className="text-center py-8">
            <div className="w-16 h-16 rounded-full bg-red-accent/10 flex items-center justify-center mx-auto mb-6">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-red-accent">
                <polyline points="20 6 9 17 4 12" />
              </svg>
            </div>
            <h2 className="text-2xl font-bold text-white mb-2">Request Sent!</h2>
            <p className="text-sm text-muted mb-8 max-w-sm mx-auto">
              Thanks for reaching out. We will review your request and get back to you within 48 hours.
            </p>
            <button
              onClick={handleClose}
...
function BookCTA({ onOpenBooking }) {
  ...
  return (
    <section id="book" ref={ref} className="relative px-6 pb-32 max-w-4xl mx-auto text-center">
      <div className={`relative p-12 md:p-16 rounded-3xl bg-gradient-to-b from-surface to-ink-3 border border-white/5 overflow-hidden transition-all duration-[900ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-white mb-4">Book an Artist</h2>
          <p className="text-muted max-w-lg mx-auto mb-8 leading-relaxed">
            Whether it is a live show, studio feature, or collaboration - reach out
            and let us make something happen.
          </p>
          <button
            onClick={onOpenBooking}
      ...
      <div className="pt-10 border-t border-white/5">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="text-sm text-muted-2">
            <span className="font-semibold text-white/50">Mocking by Jay</span> &copy; {new Date().getFullYear()}
          </div>
          <div className="flex items-center gap-6">
            {["Instagram", "Twitter / X", "SoundCloud", "YouTube"].map((social) => (
              <a key={social} href="#" className="text-xs text-muted-2 hover:text-red-accent transition-colors duration-300">{social}</a>
            ))}
          </div>
